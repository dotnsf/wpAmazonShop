	...
	if( $pdo != null ){
		...
		for( $i = 0; $i < count( $recordArray ); $i ++ ){
			...
			try{
				...
				$sql0 = "select count(*) as cnt from " . TABLE_PREFIX . "posts where post_title = '" . $title . "'";
		...
		while( list( $category, $num ) = each( $categorys ) ){
			//. Insert
			try{
				//. (未対応) カテゴリが既にDBに存在している可能性も考慮する
				$sql0 = "select count(*) as cnt from " . TABLE_PREFIX . "terms where name = '" . $category . "'";
				$stmt0 = $pdo->query( $sql0 );
				if( $row = $stmt0->fetch( PDO::FETCH_ASSOC ) ){
					$cnt = ( int )$row['cnt'];
					if( $cnt == 0 ){
						...
						$sql1 = "select " . TABLE_PREFIX . ".term_taxonomy_id as term_taxonomy_id from " . TABLE_PREFIX . "taxonomy, " . TABLE_PREFIX . "terms where " . TABLE_PREFIX . "terms.name = '" . $category . "' and " . TABLE_PREFIX . "terms.term_id = " . TABLE_PREFIX . "taxonomy.term_id";
